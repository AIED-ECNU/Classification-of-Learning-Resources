...
def convert(filePath,outPath):
    ...
    for page in PDFPage.get_pages(infile, check_extractable=True):
        ...
        with open('%s' % (outPath), 'wb') as f:
            f.write('\n'+convertedPDF.encode())#python3编码转换新方法“wb”,encode():http://pythoncentral.io/encoding-and-decoding-strings-in-python-3-x/
...
def pdfTotxt(fileDir):
    ...
    for file in files:
        ...
        print("Saved "+outPath)
pdfTotxt(u'F:\\期刊文献分析\\pdffiledir')